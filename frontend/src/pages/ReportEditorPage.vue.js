...
const report = useQuery({ queryKey: ["report", reportId], queryFn: () => api(`/api/reports/${reportId}`) });
const materials = useQuery({ queryKey: ["materials"], queryFn: () => api("/api/materials"), enabled: incrementOpen });
const qaIssues = computed(() => report.data.value?.qa_issues || []);
const currentDetails = computed(() => selected.value);
function choose(sentence) { selected.value = sentence; if (mode.value === "trace")
...
async function saveSection(section, index, event) {
    ...
    saveState.value = "正在保存";
    try {
        const result = await api(`/api/reports/${reportId}/sections`, jsonInit("PUT", { old_title: section.title, new_title: visible, section_index: index + 1 }));
...
async function finalize(force = false) {
    try {
        ...
        if (error?.payload?.error === "QA_BLOCKING" && confirm("仍有阻断性质量问题。确认在知情情况下完成审核？"))
...
function confidence(item) { return ({ high: "高", medium: "中", low: "低" }[String(item.confidence_level || "").toLowerCase()] || "需人工复核"); }
const __VLS_ctx = {
    ...{},
    ...{},
};
...
if (__VLS_ctx.report.data.value) {
    ...
    for (const [section, index] of __VLS_vFor((__VLS_ctx.report.data.value.sections))) {
        ...
        // @ts-ignore
        [report, reviewApplied, createSnapshot,];
    }
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ class: "trace-key" },
    });
    ...
    for (const [section, sectionIndex] of __VLS_vFor((__VLS_ctx.report.data.value.sections))) {
        ...
        __VLS_asFunctionalElement1(__VLS_intrinsics.h2, __VLS_intrinsics.h2)({
            ...{ onBlur: (...[$event]) => {
                    ...
                    // @ts-ignore
                    [report, report, saveTitle, saveSection,];
                } },
            ...
            for (const [sentence] of __VLS_vFor((paragraph.sentences))) {
                ...
                if (sentence.source_level === 'SUBHEADING') {
                    __VLS_asFunctionalElement1(__VLS_intrinsics.h3, __VLS_intrinsics.h3)({
                        ...
                        ...{ class: ([__VLS_ctx.sourceClass(sentence), { selected: __VLS_ctx.selected?.id === sentence.id }]) },
                        contenteditable: true,
                        spellcheck: "false",
                    });
                    /** @type {__VLS_StyleScopedClasses['sentence']} */ ;
            ...
            if (paragraph.sentences.some(sentence => sentence.source_level !== 'SUBHEADING')) {
                ...
                for (const [sentence] of __VLS_vFor((paragraph.sentences.filter(sentence => sentence.source_level !== 'SUBHEADING')))) {
                    __VLS_asFunctionalElement1(__VLS_intrinsics.span, __VLS_intrinsics.span)({
                        ...
                        key: (sentence.id),
                        ...{ class: "sentence" },
                        ...{ class: ([__VLS_ctx.sourceClass(sentence), { selected: __VLS_ctx.selected?.id === sentence.id, excluded: sentence.selected === false }]) },
                        contenteditable: true,
                        spellcheck: "false",
                    });
                    /** @type {__VLS_StyleScopedClasses['sentence']} */ ;
    ...
    if (__VLS_ctx.sideTab === 'evidence') {
        ...
        if (__VLS_ctx.currentDetails) {
            ...
            for (const [source] of __VLS_vFor((__VLS_ctx.currentDetails.sources || __VLS_ctx.currentDetails.facts || []))) {
                __VLS_asFunctionalElement1(__VLS_intrinsics.section, __VLS_intrinsics.section)({
            ...
            if (!(__VLS_ctx.currentDetails.sources?.length || __VLS_ctx.currentDetails.facts?.length || __VLS_ctx.currentDetails.inferences?.length)) {
                __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
                    ...{ class: "empty" },
                });
            ...
            __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({});
            __VLS_asFunctionalElement1(__VLS_intrinsics.strong, __VLS_intrinsics.strong)({});
            (__VLS_ctx.mode === 'trace' ? '查看对应事实、原文和分析推论。' : '切换到溯源模式后点击正文。');
        }
    }
    else {
        __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...
        for (const [issue, index] of __VLS_vFor((__VLS_ctx.qaIssues))) {
            ...
            // @ts-ignore
            [mode, currentDetails, currentDetails, currentDetails, qaIssues,];
        }
