...
const detail = useQuery({
  ...
  queryFn: () => api<any>(`/api/material-comparisons/${selectedId.value}?compact=true`),
...
const selectedDetail = useQuery({
  ...
  queryFn: () => api<any>(`/api/material-comparisons/${selectedId.value}/items/${selectedItemId.value}`),
...
const selectedItem = computed(() => selectedDetail.data.value
...
async function review(item: any, status: string) {
  ...
  await Promise.all([
    ...
    qc.invalidateQueries({ queryKey: ["material-comparison-item", selectedId, selectedItemId] }),
  ]);
}
...
<template>
  <section class="comparison-workspace" :class="{ embedded }">
    ...
    <div class="comparison-shell" :class="{ 'with-runs': !embedded }">
      ...
      <main v-if="detail.data.value" class="result-workspace">
        ...
        <div v-else class="review-layout">
          ...
          <aside class="evidence-panel">
            <template v-if="selectedItem"><div class="evidence-head"><span class="relation-badge" :class="selectedItem.change_type">{{ labels[selectedItem.change_type] || selectedItem.change_type }}</span><small>{{ selectedItem.confidence === "high" ? "高" : selectedItem.confidence === "low" ? "低" : "中" }}置信度</small></div><h3>{{ selectedItem.title }}</h3><p v-if="selectedDetail.isFetching.value" class="detail-loading">正在读取来源与原文…</p><p v-else class="rationale">{{ selectedItem.rationale }}</p>
              <div class="fact-block baseline"><small>报告原有事实</small><p>{{ selectedItem.evidence?.baseline_fact?.content || "报告中没有对应旧事实，这是独立新增信息。" }}</p><div v-if="selectedItem.evidence?.baseline_fact" class="provenance-list"><div v-for="source in selectedItem.evidence?.baseline_fact?.evidence || []" :key="source.evidence_id || source.unit_id" class="provenance-row"><div class="source-location"><span>旧材料位置</span><b>{{ evidenceLabel(source) }}</b></div><blockquote v-if="quoteAddsDetail(selectedItem.evidence?.baseline_fact?.content, source.quote)">{{ source.quote }}</blockquote><details v-else><summary>查看原文</summary><blockquote>{{ source.quote }}</blockquote></details></div><p v-if="!selectedItem.evidence?.baseline_fact?.evidence?.length" class="muted">该历史版本没有保存旧材料来源定位。</p></div></div>
              <div class="fact-block current"><small>新材料事实</small><p>{{ selectedItem.evidence?.new_fact?.content }}</p><div class="provenance-list"><div v-for="source in selectedItem.evidence?.new_fact?.evidence || []" :key="source.evidence_id || source.unit_id" class="provenance-row"><div class="source-location"><span>新材料位置</span><b>{{ evidenceLabel(source) }}</b></div><blockquote v-if="quoteAddsDetail(selectedItem.evidence?.new_fact?.content, source.quote)">{{ source.quote }}</blockquote><details v-else><summary>查看原文</summary><blockquote>{{ source.quote }}</blockquote></details></div><p v-if="!selectedItem.evidence?.new_fact?.evidence?.length" class="muted">该历史对比项尚未取得可展示的原文定位。</p></div></div>
      ...
      <div v-else-if="detail.isLoading.value" class="comparison-loading" aria-live="polite"><div class="loading-line wide"></div><div class="loading-line"></div><div class="loading-line short"></div><span>正在载入变化索引与报告正文…</span></div>
      <div v-else-if="detail.isError.value" class="empty"><div><strong>对比结果加载失败</strong>请稍后重试，已完成的分析结果不会丢失。</div></div>
      <div v-else class="empty"><div><strong>{{ embedded ? "尚未形成对比结果" : "选择一次对比" }}</strong>{{ embedded ? "完成分析后将在这里逐句展示材料变化。" : "查看新材料相对于报告基线带来的变化。" }}</div></div>
...
.provenance-list{margin-top:12px;padding-top:10px;border-top:1px solid var(--color-border)}.provenance-row+.provenance-row{margin-top:10px;padding-top:10px;border-top:1px dashed var(--color-border)}.source-location{display:grid;gap:2px}.source-location span{color:var(--color-faint);font-size:11px}.source-location b{color:var(--color-muted);font-size:12px;font-weight:600}.provenance-row blockquote{margin:8px 0 0;padding:8px 10px;border-left:2px solid #aebbc9;background:#fff;color:var(--color-muted);line-height:1.55}.provenance-row details{margin-top:7px;color:var(--color-primary);font-size:12px}.provenance-row details blockquote{color:var(--color-muted);font-size:13px}.provenance-row summary{cursor:pointer}.provenance-row summary:hover{text-decoration:underline}
.comparison-loading{display:grid;align-content:start;gap:12px;min-height:280px;padding:34px;background:#fff;color:var(--color-muted)}.comparison-loading span{margin-top:6px;font-size:13px}.loading-line{width:58%;height:12px;background:#edf0f3;animation:loading-pulse 1.4s ease-in-out infinite}.loading-line.wide{width:82%;height:18px}.loading-line.short{width:36%}.detail-loading{padding:10px 0;color:var(--color-muted);font-size:13px}@keyframes loading-pulse{0%,100%{opacity:.55}50%{opacity:1}}
</style>
