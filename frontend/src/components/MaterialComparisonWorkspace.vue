  ...
  <section class="comparison-workspace" :class="{ embedded }">
    ...
    <div class="comparison-shell" :class="{ 'with-runs': !embedded }">
      ...
      <main v-if="detail.data.value" class="result-workspace">
        <div class="result-head"><div><small>对比基线 · 版本 {{ detail.data.value.baseline?.version_label || detail.data.value.base_version_id }}</small><h2>{{ detail.data.value.baseline?.title || "报告材料变化" }}</h2></div><div class="summary-strip"><div><strong>{{ detail.data.value.summary?.new_fact_count || 0 }}</strong><span>新事实</span></div><div><strong>{{ detail.data.value.summary?.change_counts?.conflict || 0 }}</strong><span>冲突</span></div><div><strong>{{ detail.data.value.summary?.affected_sections?.length || 0 }}</strong><span>涉及章节</span></div></div></div>
        ...
        <div v-else class="review-layout">
          <aside class="relation-panel">
            <h3>变化类型</h3><button :class="{ active: relationFilter === 'all' }" @click="relationFilter = 'all'"><span>全部有效变化</span><b>{{ items.filter(x => x.change_type !== 'irrelevant').length }}</b></button>
          ...
          <article class="baseline-document">
            <div class="document-note"><span>基线报告正文</span><small>有颜色的句子表示新材料与其存在事实关联；点击句子查看原始材料。</small></div>
            <section v-for="section in documentSections" :key="section.title"><h2>{{ section.title }}</h2><p v-for="paragraph in section.paragraphs" :key="paragraph.id"><span v-for="sentence in paragraph.sentences" :key="sentence.id" class="report-sentence" :class="[changesFor(sentence)[0]?.change_type, { affected: changesFor(sentence).length, selected: changesFor(sentence).some((x:any) => x.item_id === selectedItemId) }]" :data-change-item="changesFor(sentence)[0]?.item_id" @click="selectSentence(sentence)">{{ sentence.text }}<sup v-if="changesFor(sentence).length">{{ changesFor(sentence).length }}</sup></span></p></section>
            <div v-if="!documentSections.length" class="empty"><div><strong>基线版本尚无可定位正文</strong>变化仍可在左侧按事实逐条审阅。</div></div>
          </article>
