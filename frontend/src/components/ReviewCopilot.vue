...
watch(agentPending, (isPending) => {
  if (!isPending && error.value.startsWith("助手仍在处理当前问题")) error.value = "";
});
...
const fieldLabels: Record<string, string> = {
  ...
  required_structure: "目录结构", required_chapter_count: "章节数量",
  required_dimensions: "确认后的分析维度", required_dimension_count: "分析维度数量",
...
const toolLabels: Record<string, string> = {
  ...
  revise_analysis_plan: "调整分析规划", recheck_fact: "复核事实与证据",
...
function proposalDiffRows(proposal: any) {
  ...
  const isStructureChange = Array.isArray(after.required_structure) && after.required_structure.length > 0;
  return Object.keys(after).filter((key) => fieldLabels[key] && !(isStructureChange && key === "instruction")).map((key) => {
    const beforeValue = key === "instruction"
      ? (before.content || before.title || before.instruction)
      : key === "required_structure" ? (before.structure || before.chapter_plans) : before[key];
    const beforeChapterCount = Array.isArray(before.structure)
      ? before.structure.length
      : Array.isArray(before.chapter_plans) ? before.chapter_plans.length : 0;
    const beforeText = key === "required_chapter_count"
      ? (beforeChapterCount ? `${beforeChapterCount} 章` : "尚无目录")
      : diffValue(beforeValue, key === "instruction" ? "当前选定范围" : "尚无内容");
    const afterText = key === "required_chapter_count"
      ? `${Number(after[key]) || after.required_structure?.length || 0} 章`
      : key === "tool_name" ? (toolLabels[String(after[key])] || String(after[key])) : diffValue(after[key], "尚无内容");
    return {
...
function proposalKind(proposal: any) {
  ...
  if (["recheck_fact", "recheck_inference"].includes(tool)) return "verification";
  if (["pause_task", "resume_task", "retry_task"].includes(tool)) return "control";
  return "generic";
}
function titleList(value: any) {
  const source = Array.isArray(value?.structure) ? value.structure
    : Array.isArray(value?.chapter_plans) ? value.chapter_plans : [];
  return source.map((item: any) => typeof item === "object" ? item?.title : item).filter(Boolean);
}
function numberedText(values: any[]) {
  return values.map((item, index) => `${index + 1}. ${String(item)}`).join("\n");
}
function proposalDisplayRows(proposal: any) {
  ...
  if (kind === "structure") {
    ...
    const nextTitles = Array.isArray(after.required_structure) ? after.required_structure : [];
    const count = Number(after.required_chapter_count || args.required_chapter_count || nextTitles.length || 0);
    return [{
      key: "structure", label: "报告目录", before: numberedText(currentTitles) || "尚未形成目录",
  ...
  if (kind === "requirements") return [
    ...
    { key: "requirements", label: "完整报告要求", before: diffValue(before.requirements, "尚未填写"), after: diffValue(after.requirements, "保持不变"), beforeLabel: "当前", afterLabel: "调整后" },
  ];
  if (kind === "analysis") return [{
    key: "analysis", label: "分析规划要求",
    before: diffValue(before.dimensions || before.chapter_plans, "沿用当前分析规划"),
    after: [
      String(after.instruction || args.instruction || "按本轮要求重新规划"),
      Array.isArray(after.required_dimensions) && after.required_dimensions.length
        ? `确认维度：${after.required_dimensions.join("、")}` : "",
      Number(after.required_dimension_count || 0) ? `维度数量：${after.required_dimension_count}` : "",
    ].filter(Boolean).join("\n"), beforeLabel: "当前依据", afterLabel: "确认目标",
  }];
  if (kind === "content") return [{
    key: "content", label: toolLabels[proposalToolName(proposal)] || "正文修改",
    before: diffValue(before.content || before.paragraph || before.title || before.section, "当前选定内容"),
    after: String(after.instruction || args.instruction || "按本轮要求修改"),
    beforeLabel: "修改范围", afterLabel: "修改要求",
  }];
  if (kind === "verification") return [{
    key: "verification", label: proposalToolName(proposal) === "recheck_fact" ? "事实与证据复核" : "分析判断复核",
    before: diffValue(before.content, "当前选定内容"),
    after: String(after.instruction || args.instruction || "重新核验依据"),
    beforeLabel: "待复核内容", afterLabel: "复核问题",
  }];
  if (kind === "control") return [{
    key: "control", label: "任务操作", before: "保持当前任务状态",
    after: toolLabels[proposalToolName(proposal)] || "执行任务操作", beforeLabel: "当前", afterLabel: "确认后",
  }];
  return proposalDiffRows(proposal).map((row: any) => ({ ...row, beforeLabel: "当前", afterLabel: "调整后" }));
}
  ...
  if (thread.value?.status === "open") {
    ...
    try {
      ...
      error.value = e?.payload?.error === "INTERACTION_THREAD_BUSY"
        ? "助手仍在处理当前问题，完成后再开始新对话。" : e.message || "无法归档当前会话";
      return;
    }
  }
  thread.value = null;
...
async function archiveCurrent() {
  ...
  try {
    ...
    error.value = e?.payload?.error === "INTERACTION_THREAD_BUSY"
      ? "助手仍在处理当前问题，完成后再归档。" : e.message || "无法归档会话";
  }
}
</script>
  ...
  <section class="copilot" :class="{ compact }">
    <header class="copilot-toolbar">
      <div class="session-title">
        ...
        <small v-if="thread?.id">{{ thread.status === "closed" ? "已归档，只读" : decisionSummary || "修改前需确认提案" }}</small>
            ...
            <article v-else class="proposal conversation-proposal">
              ...
              <template v-if="proposalDisplayRows(item).length && proposalExpanded(item)">
                <div class="diff-preview">
                  <section v-for="row in proposalDisplayRows(item)" :key="row.key">
                    <b>{{ row.label }}</b>
                    <div class="diff-side before"><small>{{ row.beforeLabel }}</small><del>{{ row.before }}</del></div>
                    <div class="diff-side after"><small>{{ row.afterLabel }}</small><ins>{{ row.after }}</ins></div>
                  </section>
                </div>
...
.proposal-list{display:grid;gap:8px;padding:8px}.proposal{padding:13px;border:1px solid var(--color-border);background:#fff}.proposal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.proposal-head>div{min-width:0}.proposal-head p{display:-webkit-box;overflow:hidden;margin:3px 0 0;color:var(--color-muted);line-height:1.5;-webkit-line-clamp:3;-webkit-box-orient:vertical}.changed-fields{display:flex;flex-wrap:wrap;gap:5px;margin-top:9px}.changed-fields span{padding:2px 6px;border:1px solid var(--color-border);border-radius:3px;background:#fff;color:var(--color-text-secondary);font-size:11px}.changed-fields .impact-chip{border-color:#cddbeb;background:#f1f6fb;color:#315f8f}.proposal-toggle{margin-top:8px;padding:0;border:0;background:transparent;color:var(--color-primary);font-size:12px}.diff-preview{display:grid;gap:12px;max-height:340px;margin:10px 0;padding-right:4px;overflow:auto;overscroll-behavior:contain}.diff-preview section{display:grid;gap:6px}.diff-preview section>b{font-size:11px;color:var(--color-text-secondary)}.diff-side{display:grid;grid-template-columns:60px minmax(0,1fr);align-items:start}.diff-side>small{padding:9px 7px;color:var(--color-muted);font-size:10px}.diff-side del,.diff-side ins{padding:8px 10px;text-decoration:none;white-space:pre-wrap;overflow-wrap:anywhere}.diff-side.before{background:#fafafa}.diff-side.before del{color:#5d626a}.diff-side.after{border-left:2px solid #5b8f68;background:#edf7ef}.diff-side.after ins{color:#2f6740}.proposal-boundary{display:block;padding:8px 10px;border-left:2px solid var(--color-primary);background:var(--color-primary-soft);color:var(--color-text-secondary)}.proposal-state{display:grid;gap:6px;margin-top:10px}.proposal-state small{color:var(--color-muted)}.proposal-state a{width:max-content;color:var(--color-primary);font-size:12px;font-weight:650;text-decoration:none}.proposal-state a:hover{text-decoration:underline}
